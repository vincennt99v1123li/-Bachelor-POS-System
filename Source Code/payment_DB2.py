...
class Gui:
    def __init__(self, root,id):
        '''
        set up the screen /initialize the program
        '''
        ...
        self.pageMain = ttk.Frame(self.nb)
        self.pageUpdate = ttk.Frame(self.nb)
        self.pageAdd = ttk.Frame(self.nb)
        self.pageDelete = ttk.Frame(self.nb)
        self.pageCash = ttk.Frame(self.nb)
        self.pageReport = ttk.Frame(self.nb)
        self.pageHelp = ttk.Frame(self.nb)
        ...
        self.nb.add(self.pageMain, text='Main')        
        self.nb.add(self.pageUpdate, text='Update product')
        self.nb.add(self.pageAdd, text='Add product')
        self.nb.add(self.pageDelete, text='Delete product')
        self.nb.add(self.pageCash, text='Cash in machine')
        self.nb.add(self.pageReport, text='Report & reset')
        self.nb.add(self.pageHelp, text='Help')
        
       
        '''
        setup main in POS
        '''
        self.label = tk.Label(self.pageMain, bg='grey93', text='enter product code')
        self.label.pack()
        
        self.editArea2 = tkst.ScrolledText(self.pageMain,height=5)
        self.editArea2.pack( fill="both") 
        self.editArea2.config(state="disabled")
        
        self.editArea = tkst.ScrolledText(self.pageMain,height=5)
        ...
        self.frame4 = tk.Frame(self.pageMain)
        
        self.frame4.pack()
        
        self.frame5 = tk.Frame(self.pageMain)
        
        self.frame5.pack()
        
        self.frame6 = tk.Frame(self.pageMain)
        
        self.frame6.pack()

        self.frame7 = tk.Frame(self.pageMain)
        ...
        self.updateLabel1 = tk.Label(self.pageUpdate, bg='grey93',text = 'Please input the Product ID if you want to update that Product :\n If subtype not related to type, the subtype uploaded will be empty')
        self.updateLabel1.place(x=5,y=5)
        
        self.updateLabel2 = tk.Label(self.pageUpdate, bg='grey93',text = 'Product ID :')
        self.updateLabel2.place(x = 5,y = 55)
        self.updateEntry2 = tk.Entry(self.pageUpdate,width= 15)
        self.updateEntry2.place(x = 100, y = 55)
        
        self.updateSearch=tk.Button(self.pageUpdate, text='Search',command=lambda: self.showData())
        self.updateSearch.place(x = 250, y = 55)
        self.updateSearch.config(state=NORMAL)
        
        tk.Label(self.pageUpdate, bg='grey93',text = '-'*89).place(x = 5, y = 105)
        
        self.updateLabel3 = tk.Label(self.pageUpdate, bg='grey93',text = ' ')
        self.updateLabel3.place(x=5,y=155)
        
        self.updateLabel4 = tk.Label(self.pageUpdate, bg='grey93',text = 'Which item you what to alter?')
        self.updateLabel4.place(x = 350,y = 200)
        type1 = tk.StringVar()
        self.updateOption1= ttk.Combobox(self.pageUpdate, width=12, textvariable=type1)
        self.updateOption1['values'] = ('product_name', 'product_brand', 'product_price', 'quantity', 'product_type', 'subtype') 
        self.updateOption1.grid(column=1, row=1)
        self.updateOption1.current(0)
        self.updateOption1.place(x = 350, y = 225)
        
        
        self.updateLabel6 = tk.Label(self.pageUpdate, bg='grey93',text = 'New data:')
        self.updateLabel6.place(x = 350,y = 265)
        self.updateEntry6 = tk.Entry(self.pageUpdate,width= 15)
        self.updateEntry6.place(x = 350, y = 290)
        self.updateEntry6.config(state=NORMAL)
        tk.Label(self.pageUpdate, bg='grey93',text = '< Name, Brand, Price, Quantity').place(x = 500, y = 290)
        
        
        type4 = tk.StringVar()
        self.updateOption4= ttk.Combobox(self.pageUpdate, width=12, textvariable=type4)
        self.updateOption4['values'] = ('Drink','Food'  ) 
        self.updateOption4.grid(column=1, row=2)
        self.updateOption4.current(0)
        self.updateOption4.place(x = 350, y = 330)
        self.updateOption4.config(state=NORMAL)
        tk.Label(self.pageUpdate, bg='grey93',text = '< Type').place(x = 500, y = 330)
        

        
        type2 = tk.StringVar()
        self.updateOption2= ttk.Combobox(self.pageUpdate, width=12, textvariable=type2)
        self.updateOption2['values'] = ('','Bread','Cup noodle','Ice cream', 'Frozen food', 'Snack' ) 
        self.updateOption2.grid(column=1, row=2)
        self.updateOption2.current(0)
        self.updateOption2.place(x = 350, y = 360)
        self.updateOption2.config(state=NORMAL)
        tk.Label(self.pageUpdate, bg='grey93',text = '< Subtype (Food)').place(x = 500, y = 360)
        
       
        
        type3 = tk.StringVar()
        self.updateOption3= ttk.Combobox(self.pageUpdate, width=12, textvariable=type3)
        self.updateOption3['values'] = ('','Tea','Carbonated drink','Other','Coffee','Juice','Beer'  ) 
        self.updateOption3.grid(column=1, row=2)
        self.updateOption3.current(0)
        self.updateOption3.place(x = 350, y = 390)
        self.updateOption3.config(state=NORMAL)
        tk.Label(self.pageUpdate, bg='grey93',text = '< Subtype (Drink)').place(x = 500, y = 390)
        
        
        self.updateSubmit=tk.Button(self.pageUpdate, text='submit', \
            command=lambda: self.alter())
        self.updateSubmit.place(x = 350, y=430)
        self.updateSubmit.config(state=NORMAL)
        
       
        
        self.updateLabel7= tk.Label(self.pageUpdate, bg='grey93',text = 'Status : Ready')
        self.updateLabel7.place(x = 420,y = 430)
        
        self.updateUnlock=tk.Button(self.pageUpdate, text='Unlock / Start',command=lambda: self.buttonState(2))
        ...
        self.updateProductType=''
        
        '''
        setup add page
        '''
        
        self.addLabel0 = tk.Label(self.pageAdd, bg='grey93',text = 'Please input with the following data to add Product. \n If subtype not related to type, the subtype uploaded will be empty')
        self.addLabel0.place(x=5,y=5)

        
        self.addLabel2 = tk.Label(self.pageAdd, bg='grey93',text = 'Product Name :')
        self.addLabel2.place(x = 5,y = 105)
        self.addEntry2 = tk.Entry(self.pageAdd,width= 25)
        self.addEntry2.place(x = 115, y = 105)
        
        self.addLabel3 = tk.Label(self.pageAdd, bg='grey93',text = 'Product Brand :')
        self.addLabel3.place(x = 5,y = 155)
        self.addEntry3 = tk.Entry(self.pageAdd,width= 25)
        self.addEntry3.place(x = 115, y = 155)
        
        self.addLabel4 = tk.Label(self.pageAdd, bg='grey93',text = 'Product Price :')
        self.addLabel4.place(x = 5,y = 205)
        self.addEntry4 = tk.Entry(self.pageAdd,width= 25)
        self.addEntry4.place(x = 115, y = 205)
        
        self.addLabel5 = tk.Label(self.pageAdd, bg='grey93',text = 'Quanitiy :')
        self.addLabel5.place(x = 5,y = 255)
        self.addEntry5 = tk.Entry(self.pageAdd,width= 25)
        self.addEntry5.place(x = 115, y = 255)
        
        self.addLabel6 = tk.Label(self.pageAdd, bg='grey93',text = 'Type :')
        self.addLabel6.place(x = 5,y = 305)
        
        
        self.addOption1= ttk.Combobox(self.pageAdd, width=12, textvariable=type)
        ...
        type5 = tk.StringVar()
        self.addLabel7 = tk.Label(self.pageAdd, bg='grey93',text = 'Subtype :')
        self.addLabel7.place(x = 5,y = 355)
        self.addOption2= ttk.Combobox(self.pageAdd, width=12, textvariable=type5)
        self.addOption2['values'] = (' ','Bread','Cup noodle','Ice cream', 'Frozen food', 'Snack' ) 
        self.addOption2.grid(column=1, row=2)
        self.addOption2.current(0)
        self.addOption2.place(x = 115, y = 355)
        
        tk.Label(self.pageAdd, bg='grey93',text = '/').place(x = 250,y = 355)
        
        type6 = tk.StringVar()
        self.addOption3= ttk.Combobox(self.pageAdd, width=12, textvariable=type6)
        self.addOption3['values'] = (' ','Tea','Carbonated drink','Other','Coffee','Juice','Beer'  ) 
        self.addOption3.grid(column=1, row=2)
        self.addOption3.current(0)
        self.addOption3.place(x = 270, y = 355)
            
        self.addSubmit=tk.Button(self.pageAdd, text='submit', \
            command=lambda: self.adding())
        self.addSubmit.place(x = 20, y = 405)
        self.addSubmit.config(state=NORMAL)
        
        self.addLabel8= tk.Label(self.pageAdd, bg='grey93',text = 'Status : Ready')
        self.addLabel8.place(x = 115,y = 405)
        
        self.addUnlock=tk.Button(self.pageAdd, text='Unlock / Start',command=lambda: self.buttonState(2))
        self.addUnlock.place(x = 5, y = 500)
        self.addUnlock.config(state=DISABLED)
        
        self.addDisplay= tk.Label(self.pageAdd, bg='grey93',text = '')
        self.addDisplay.place(x = 500,y = 100)
        ...
        self.addOption3.config(state=NORMAL)
        '''
        setup delete page
        '''
        self.delLabel1 = tk.Label(self.pageDelete, bg='grey93',text = 'Please input the Product ID if you want to delete that Product : ')
        self.delLabel1.place(x=5,y=5)
        
        self.delLabel2 = tk.Label(self.pageDelete, bg='grey93',text = 'Product ID :')
        self.delLabel2.place(x = 5,y = 55)
        self.delEntry2 = tk.Entry(self.pageDelete,width= 15)
        self.delEntry2.place(x = 100, y = 55)
        
        self.delLabel3 = tk.Label(self.pageDelete, bg='grey93',text = ' ')
        self.delLabel3.place(x=5,y=155)
        
        
        self.delSearch=tk.Button(self.pageDelete, text='Search', \
            command=lambda: self.deleteShowData())
        self.delSearch.place(x = 250, y = 55)
        self.delSearch.config(state=NORMAL)
        
        self.delLabel8 = tk.Label(self.pageDelete, bg='grey93',text = 'Password :')
        self.delLabel8.place(x = 5,y = 405)
        self.delEntry3 = tk.Entry(self.pageDelete,width= 15)
        self.delEntry3.place(x = 100, y = 405)
        
        self.delDel=tk.Button(self.pageDelete, text='Delete', \
            command=lambda: self.delete())
        self.delDel.place(x = 20, y=455)
        self.delDel.config(state=NORMAL)   
        
        self.delLabel7= tk.Label(self.pageDelete, bg='grey93',text = 'Status : Ready')
        self.delLabel7.place(x = 115,y = 455)
        
        self.delUnlock=tk.Button(self.pageDelete, text='Unlock / Start',command=lambda: self.buttonState(2))
        ...
        self.delEntry3.config(state=NORMAL)
        self.delProductType=''
        ...
        self.cashLabel1 = tk.Label(self.pageCash, bg='grey93',text = 'Please input the total value of cash initially:')
        self.cashLabel1.place(x=5,y=5)
        
       
        self.cashEntry = tk.Entry(self.pageCash,width= 15)
        self.cashEntry.place(x = 5, y = 50)
        self.cashEntry.config(state=NORMAL)
        
        self.cashConfirm1=tk.Button(self.pageCash, text='Confirm', \
            command=lambda: self.cashInMachine(1) )
        self.cashConfirm1.place(x = 155, y=50)
        self.cashConfirm1.config(state=NORMAL)
        
        
        self.cashChange1=tk.Button(self.pageCash, text='Change', \
            command=lambda: self.cashInMachineChange(1))
        self.cashChange1.place(x = 220, y=50)
        self.cashChange1.config(state=DISABLED)
        
        
        fileRead = open('cashInMachineInitial.txt', 'r')
        text1=str(fileRead.readline())
        
        fileRead.close()
        self.cashNowLabelInitial = tk.Label(self.pageCash, bg='grey93',text = 'now: '+text1)
        self.cashNowLabelInitial.place(x=280,y=50)
            
        self.cashLabel2 = tk.Label(self.pageCash, bg='grey93',text = 'Please input the total value of cash finally:')
        self.cashLabel2.place(x=5,y=100)
        
       
        self.cashEntry2 = tk.Entry(self.pageCash,width= 15)
        self.cashEntry2.place(x = 5, y = 150)
        
        
        
        
        self.cashConfirm2=tk.Button(self.pageCash, text='Confirm', \
            command=lambda: self.cashInMachine(2))
        self.cashConfirm2.place(x = 155, y=150)
        self.cashConfirm2.config(state=NORMAL)
        
        self.cashChange2=tk.Button(self.pageCash, text='Change', \
            command=lambda: self.cashInMachineChange(2))
        self.cashChange2.place(x = 220, y=150)
        self.cashChange2.config(state=DISABLED)
        
        fileRead2 = open('cashInMachineNow.txt', 'r')
        text2=str(fileRead2.readline())
        
        fileRead2.close()
        
        self.cashNowLabelThen = tk.Label(self.pageCash, bg='grey93',text =  'now: '+text2)
        self.cashNowLabelThen.place(x=280,y=150)
        
        self.cashUnlock=tk.Button(self.pageCash, text='Unlock / Start',command=lambda: self.buttonState(2))
        self.cashUnlock.place(x = 5, y = 500)
        self.cashUnlock.config(state=DISABLED)
        
        self.cashLabel3= tk.Label(self.pageCash, bg='grey93',text='')
        ...
        '''
        tk.Label(self.pageReport, bg='grey93',text='Print Z report:').place(x = 5, y=5)
        tk.Button(self.pageReport,text='Z report',command=lambda: self.Z()).place(x = 5, y=50)
        
        self.zLabel=tk.Label(self.pageReport, bg='grey93',text='ready')
        self.zLabel.place(x = 70, y=50)
        
        tk.Label(self.pageReport, bg='grey93',text='Create bar chart to show how many product have been sold today:').place(x = 5, y=100)
        tk.Button(self.pageReport,text='Bar chart',command=lambda: self.barChart()).place(x = 5, y=150)
       
        
        tk.Label(self.pageReport, bg='grey93',text='Reset the whole system:').place(x = 5, y=200)
        self.btnReset=tk.Button(self.pageReport,text='Reset',command=lambda: self.buttonStateChange())
        self.btnReset.place(x = 5, y=250)
        self.btnResetConfirm=tk.Button(self.pageReport,text='Confirm',command=lambda: self.reset())
        ...
        setup help page
        '''
        self.editArea3 = tkst.ScrolledText(self.pageHelp,height=5)
        self.editArea3.pack( expand=1,fill="both") 
        self.editArea3.insert(tk.INSERT, 'In main: click the code frist then click the function key \nIn update/ add product: If subtype not related to type, the subtype uploaded will be empty')
    ...
    def pressed_button(self,number):
        ...
        if 'x' not in self.list and self.xFlag2==False:
            self.xFlag= False  
    ...
    def buttonState(self,number):
        '''
        allow type by keyboard in main
        '''
        if number == 1:
            self.keyOnFlag=True
            ...
            self.updateOption2.config(state=DISABLED)
            self.updateOption3.config(state=DISABLED)
            self.updateOption4.config(state=DISABLED)
            ...
            self.addOption3.config(state=DISABLED)
            self.addSubmit.config(state=DISABLED)
            
            self.delEntry2.config(state=DISABLED)
            self.delEntry3.config(state=DISABLED)
    ...
    def pay(self):
        '''
        ask user to input how much customer should pay
        '''
        self.btnCancel.config(state=DISABLED)
        if self.total >=0 :
    ...
    def receive(self):   
        ...
        try:
            ...
            self.editArea2.config(state="normal")
            self.editArea2.delete(1.0, tk.END) 
            self.editArea2.insert(tk.INSERT ,'invalid input') 
            self.editArea2.insert(tk.INSERT ,'\n')
            self.editArea2.config(state="disabled")
            self.pay()
    ...
    def Z(self):
        ...
        fileIn.write('Balance: ')   
        fileIn.write(str(round(float(cashFinal)-float(cashInitial)-pSalesTotal,2)))
        ...
        self.zLabel.config(text=datetime.datetime.now())
        ...
        with self.connection.cursor() as cursor:
            ...
            for e in cursor.fetchall():
                ...
                if str(e['product_id'])==self.updateEntry2.get():
                    ...
                    self.updateProductType=e['product_type']
                    #print(e['product_type'])
            if found == True and self.updateProductType=='Drink':
                
                cursor.execute('SELECT * from drink where 1')
                for e in cursor.fetchall():
                    if str(e['product_id'])==self.updateEntry2.get():
                        pText=pText+"Subtype:"+e['subtype']+'\r\n'
                        
                        self.updateLabel3.config(text=pText)
                    
                
            if found == True and self.updateProductType=='Food':
                
                cursor.execute('SELECT * from food where 1')
                for e in cursor.fetchall():
                    if str(e['product_id'])==self.updateEntry2.get():
                        pText=pText+"Subtype:"+e['subtype']+'\r\n'
    ...
    def alter(self):
        with warnings.catch_warnings():
            ...
            try:
                if self.updateOption1.get() == 'product_type':
                    with self.connection.cursor() as cursor:
                        
                        cursor.execute("UPDATE product SET %s= '%s' where product_id=%d "%(self.updateOption1.get(),self.updateOption4.get(),int(self.updateEntry2.get())))
                        self.connection.commit()
                        
                        if self.updateProductType=="Drink" and self.updateOption4.get() == "Drink":
                            cursor.execute("DELETE FROM drink WHERE drink.product_id=%d "%(int(self.updateEntry2.get())))
                            self.connection.commit()
                            cursor.execute("INSERT INTO `Drink` (`product_id`, `subtype`) VALUES (%d, '')"%int(self.updateEntry2.get()))
                            self.connection.commit()
                            self.updateLabel7.config(text='Status : Success') 
                        elif self.updateProductType=="Food" and self.updateOption4.get() == "Food":
                            cursor.execute("DELETE FROM food WHERE food.product_id=%d "%(int(self.updateEntry2.get())))
                            self.connection.commit()
                            cursor.execute("INSERT INTO `Food` (`product_id`, `subtype`) VALUES (%d, '')"%int(self.updateEntry2.get()))
                            self.connection.commit()
                            self.updateLabel7.config(text='Status : Success') 
                        elif self.updateProductType=="Food" and self.updateOption4.get() == "Drink":
                            cursor.execute("DELETE FROM food WHERE food.product_id=%d "%(int(self.updateEntry2.get())))
                            self.connection.commit()
                            cursor.execute("INSERT INTO `Drink` (`product_id`, `subtype`) VALUES (%d, '')"%int(self.updateEntry2.get()))
                            self.connection.commit()
                            self.updateLabel7.config(text='Status : Success') 
                        elif self.updateProductType=="Drink" and self.updateOption4.get() == "Food":
                            cursor.execute("DELETE FROM drink WHERE drink.product_id=%d "%(int(self.updateEntry2.get())))
                            self.connection.commit()
                            cursor.execute("INSERT INTO `Food` (`product_id`, `subtype`) VALUES (%d, '')"%int(self.updateEntry2.get()))
                            self.connection.commit()
                            ...
                            self.updateLabel7.config(text='Status : Error') 
                       
                elif self.updateOption1.get() == 'subtype':
                    with self.connection.cursor() as cursor:
                        
                        if self.updateProductType=="Drink":
                            cursor.execute("UPDATE drink SET %s= '%s' where product_id=%d "%(self.updateOption1.get(),self.updateOption3.get(),int(self.updateEntry2.get())))
                            self.connection.commit()
                            self.updateLabel7.config(text='Status : Success') 
                        
                           
                        
                        elif self.updateProductType=="Food":
                            cursor.execute("UPDATE food SET %s= '%s' where product_id=%d "%(self.updateOption1.get(),self.updateOption2.get(),int(self.updateEntry2.get())))
                            self.connection.commit()
                            self.updateLabel7.config(text='Status : Success') 
                    ...
                    with self.connection.cursor() as cursor:
                        ...
                        cursor.execute("UPDATE product SET %s= %f where product_id=%d "%(self.updateOption1.get(),float(self.updateEntry6.get()),int(self.updateEntry2.get())))
                        self.connection.commit()
                        self.updateLabel7.config(text='Status : Success') 
    ...
    def adding(self):
        try:
            if float(self.addEntry4.get()) <0 or int(self.addEntry5.get()) <0:
                raise ValueError
            with self.connection.cursor() as cursor:
                cursor.execute('''INSERT INTO product (`product_id`, `product_name`, `product_brand`, `product_price`, `quantity`, `product_type`) VALUES(%d,'%s','%s',%f,%d,"%s"'''%(int(0),self.addEntry2.get(),self.addEntry3.get(),float(self.addEntry4.get()),int(self.addEntry5.get()),self.addOption1.get())+')')
                cursor.execute('SELECT * from product where 1')
                
                for e in cursor.fetchall():
                    pass
                
                if self.addOption1.get() =='Food' and self.addOption2.get()!='':
                    cursor.execute("INSERT INTO food VALUES(%d,'%s')"%(e['product_id'],self.addOption2.get()))
                elif self.addOption1.get() =='Drink'and self.addOption3.get()!='':
                    cursor.execute("INSERT INTO drink VALUES(%d,'%s')"%(e['product_id'],self.addOption3.get()))
                else:
                    raise Exception
                
                self.connection.commit()
                self.addLabel8.config(text='Status : Success')
                self.showAdd(e['product_id'])
        except Exception as error:
            #print(error)
        ...
        with self.connection.cursor() as cursor:
            ...
            for e in cursor.fetchall():
                ...
                if str(e['product_id'])==str(id):
                    ...
                    #print(e['product_type'])
            if found == True and productType=='Drink':
                ...
                for e in cursor.fetchall():
                    if str(e['product_id'])==str(id):
                        ...
                        self.addDisplay.config(text=pText)
                    
                
            if found == True and productType=='Food':
            ...
            if not found:
                self.addDisplay.config(text="test")
        pass
        ...
        with self.connection.cursor() as cursor:
            ...
            for e in cursor.fetchall():
                if str(e['product_id'])==self.delEntry2.get():
                    found=True
                    self.delText="ID:"+str(e['product_id'])+'\r\n'+\
                                    "Name:"+e['product_name']+'\r\n'+\
                                    "Brand:"+e['product_brand']+'\r\n'+\
                                    "Price:"+str(e['product_price'])+'\r\n'+\
                                    "Quantity:"+str(e['quantity'])+'\r\n'+\
                    ...
                    self.delProductType=e['product_type']
                    
           
            if found == True and self.delProductType=='Drink':
                
                cursor.execute('SELECT * from drink where 1')
                for e in cursor.fetchall():
                    if str(e['product_id'])==self.delEntry2.get():
                        self.delText=self.delText+"Subtype:"+e['subtype']+'\r\n'
                        
                        self.delLabel3.config(text=self.delText)
                    
                
            if found == True and self.delProductType=='Food':
    ...
    def delete(self):
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            fileImport=open('Password.txt','r')
            passwordDelete= fileImport.readline()
            fileImport.close()
            if self.delEntry3.get() == passwordDelete:
                
                try:
                    with self.connection.cursor() as cursor:
                        if self.delProductType == 'Drink':
                            cursor.execute("DELETE FROM drink WHERE product_id ='%s'"%(int(self.delEntry2.get())))
                            self.connection.commit()
                            pass
                        elif self.delProductType == 'Food':
                            cursor.execute("DELETE FROM food WHERE product_id ='%s'"%(int(self.delEntry2.get())))
                            self.connection.commit()
                            pass
                        cursor.execute("DELETE FROM product WHERE product_id ='%s'"%(int(self.delEntry2.get())))
                        self.connection.commit()
                        self.delLabel7.config(text='Status : Success')
                        fileIn=open('deleteReport.txt','a')
                    
                        date_string=datetime.datetime.now()
                        fileIn.write('---------------------------------------------------------------------------------------------------------------')  
                        fileIn.write('\n')
                        fileIn.write('Created '+str(self.staffId)+'  '+str(date_string.strftime('%Y/%m/%d %H:%M:%S')))
                        fileIn.write('\n')
                        fileIn.write(self.delText)
                ...
                self.delLabel7.config(text='Status : Invalid password')
    ...
    def cashInMachine(self,number):
        try:
            if number == 1:
                ...
                fileRead = open('cashInMachineInitial.txt', 'r')
                text1=str(fileRead.readline())
                self.cashNowLabelInitial.config(text="now: "+text1)
                fileRead.close()
                ...
                fileRead2 = open('cashInMachineNow.txt', 'r')
                text2=str(fileRead2.readline())
                self.cashNowLabelThen.config(text="now: "+text2)
                fileRead2.close()
    ...
    def barChart(self):
        ...
        while i< len(all_product):
            ...
            if number_v!=0:
                ...
                data.append(str(all_product[i])+'\n '+pname[1:pname_length-2]+'\n  ')
        ...
        plt.bar(x, datavalue)
        plt.xticks(x, data, rotation=40, ha="right")
        plt.tight_layout()
        plt.show()
